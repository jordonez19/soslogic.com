...
class Merlin {
	...
	public function admin_page() {
		...
		wp_add_inline_style( 'merlin', '#qm-icon-container,#query-monitor-main{display:none}' ); // Hide Query Monitor plugin.
		 ...
		 wp_enqueue_style( 'merlin-dashboard-style',CRETE_URL . '/assets/css/merlin-dashboard.min.css' );
		 
		 wp_enqueue_style( 'jquery-confirm-css',CRETE_URL . '/assets/css/jquery-confirm.min.css' );
		...
		wp_enqueue_script('jquery-confirm',CRETE_URL . '/assets/js/jquery-confirm.min.js', array('jquery'), '5.0', true);
		
		wp_enqueue_script('jquery-custom-select',CRETE_URL . '/assets/js/jquery.customselect.min.js', array('jquery'), '5.0', true);
...
/**
	...
	protected function content() {
		...
		<div class="merlin__content--transition merlin__content--transition-full-wdth">
			...
			<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>
			<?php } else { ?>
			<h4 class="dh-plugin-required">Please Install and Activate Crete Core to continue import demo.</h4>
			<?php } ?>
...
/**
  ...
  /**
	...
	function elementor_update_options() {
		...
		update_option( 'elementor_cpt_support', [ 'page', 'post', 'product_template', 'footer_builder', 'edd_archive_builder', 'crete_block' ] );
	}
