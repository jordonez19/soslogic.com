...
function search_plugin_scripts_styles(){
    if (class_exists("Woocommerce")) {
        wp_register_script( 'crete-ajax-search', plugins_url('/js/crete-ajax-search.js', __FILE__ ), array('jquery'), '', true);
        wp_localize_script(
            'crete-ajax-search',
            'opt',
            array(
                'ajaxUrl'   => admin_url('admin-ajax.php'),
                'noResults' => esc_html__( 'No products found', 'crete' ),
            )
        );
    }
}
...
/*-------------------*/
    ...
    function get_product_categories_hierarchy() {
        ...
        if ( ! empty( $categories ) ) {
            ...
            return new WP_Error( 'no_categories', esc_html__( 'No categories.', 'crete' ) );
    ...
    function search_product() {
        ...
        if (isset($_POST['keyword']) && !empty($_POST['keyword'])) {
            ...
            if (!empty($query_results)) {
                ...
                $thmoptions = get_option( 'crete_options' );
                $spstyle = $thmoptions['search_product_style'];
                $pcol = $thmoptions['search_product_grid_count'];
                
                if ($spstyle == 'list'){
                    $spgrid ="crete-ajax-list-search";
                    $spli = "";
                } else {
                      $spgrid ="crete-ajax-grid-search row row-cols-1 row-cols-md-$pcol";
                      $spli = "col";
                }
                ...
                foreach ($query_results as $result) {
                    ...
                    $output .= '<li class="'.$spli.'">';
                            ...
                            $output .= '<div class="crete-ajax-product-image">';
                                $output .= '<a href="'.get_post_permalink($result->ID).'"><img src="'.esc_url(get_the_post_thumbnail_url($result->ID,'full')).'"></a>';
                            $output .= '</div>';
                            $output .= '<div class="crete-ajax-product-data">';
                            $output .= '<div class="crete-ajax-category-data">'.$categories.'</div>';
                                $output .= '<h3><a href="'.get_post_permalink($result->ID).'">'.$result->post_title.'</a></h3>';
                               
                                    $output .= '<div class="crete-ajax-product-price">';
...
/*-------------------*/
   ...
   function crete_live_search() {

    		wp_enqueue_script('crete-ajax-search');
                $catfilter = cs_get_option('search_category_ds');
                $search_style_ds = cs_get_option('search_style_ds');
               if ($catfilter=="show"){
                   $catfilcls="crete-filter-available";
               } else {
               $catfilcls="crete-filter-not-available";
               }
               
                ?>

    			<div class="product-search <?php echo esc_html($catfilcls);?> crete-search-style-<?php echo esc_html($search_style_ds);?>">
    				<form name="product-search" method="get" class="crete-search-box" action="<?php echo esc_url( home_url( '/' ) ); ?>">
    				    <?php if ($catfilter=="show"){?>
                        <?php $categories = get_product_categories_hierarchy(); ?>
                        <?php if ($categories): ?>
                        <div class="crete-ajax-search-bar category-wrapper">
                            <select name="category" class="category">
                                <option class="default" value=""><?php echo esc_html__( 'All Categories', 'crete' ); ?></option>
                        ...
                        <div class="search-wrapper">
                            <input type="text" name="s" class="search" placeholder="<?php echo esc_html__( 'Search for Product...', 'crete' ); ?>" value="<?php the_search_query(); ?>">
                         
                           <button type="submit" value="Search" class="crete-ajax-search-btn">
                    ...
                    <div class="search-results crete-search-result"></div>
        		</div>
